...
if [ -n "$DOCKERFILE" ]; then
  ...
  docker build -t testify-agent-image -f "$DOCKERFILE" "$(dirname "$DOCKERFILE")"
else
  echo "Pulling the Docker image..."
  docker pull "$DOCKER_IMAGE"
  docker tag "$DOCKER_IMAGE" testify-agent-image
fi
...
CONTAINER_ID=$(sh -c "docker run -d $ARGS testify-agent-image tail -f /dev/null")
...
# Copy the testify-agent binary into the running container
echo "Copying testify-agent binary into the container..."
docker cp dist/testify-agent "$CONTAINER_ID":/usr/local/bin/testify-agent

# Run the testify-agent command inside the running container
echo "Running the testify-agent command..."
COMMAND="/usr/local/bin/testify-agent \
