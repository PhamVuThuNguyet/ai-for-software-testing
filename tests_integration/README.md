# Integration Tests for Testify Agent
This folder contains end-to-end integration tests for Testify Agent.
...
## How to Run
You can run these example test suites using a locally hosted LLM or in the cloud just as you would normally with Testify Agent.
...
sh tests_integration/test_with_docker.sh \
  --dockerfile "testing_templates/python_fastapi/Dockerfile" \
...
```
sh tests_integration/test_with_docker.sh \
  --dockerfile "testing_templates/go_webservice/Dockerfile" \
...
```
sh tests_integration/test_with_docker.sh \
  --dockerfile "testing_templates/java_gradle/Dockerfile" \
...
```
sh tests_integration/test_with_docker.sh \
  --dockerfile "testing_templates/java_spring_calculator/Dockerfile" \
...
```
sh tests_integration/test_with_docker.sh \
  --dockerfile "testing_templates/js_vanilla/Dockerfile" \
...
The `increase_coverage.py` script attempts to run Testify Agent for all files within the `testify_agent` directory. You'll need to call a Poetry shell first before running like so:
```
poetry install
poetry shell
...
# Analyzing failures
After Testify Agent runs we store the run results in a database (see `docs/database_usage.md` for more details). 
