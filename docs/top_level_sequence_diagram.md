...
Testify Agent consists of many classes but the fundamental flow lives within the TestifyAgent and UnitTestGenerator classes. The following is a sequence diagram (written in [Mermaid](https://mermaid.js.org/syntax/sequenceDiagram.html)) depicting the flow of how Testify Agent works and interacts with a Large Language Model.

```mermaid
sequenceDiagram
    participant User
    participant TestifyAgent
    ...
    User->>TestifyAgent: Initialize with args
    TestifyAgent->>CustomLogger: Get logger instance
    TestifyAgent->>TestifyAgent: _validate_paths()
    TestifyAgent->>TestifyAgent: _duplicate_test_file()
    TestifyAgent->>UnitTestGenerator: Initialize UnitTestGenerator
    ...
    TestifyAgent->>UnitTestGenerator: initial_test_suite_analysis()
    ...
    loop Test generation and validation
        TestifyAgent->>UnitTestGenerator: generate_tests()
    ...
    TestifyAgent->>CustomLogger: Log final results
    note right of TestifyAgent: Generate report
```