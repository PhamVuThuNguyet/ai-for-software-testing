...
from testify_agent.lsp_logic.utils.utils import is_forbidden_directory, uri_to_path
...
async def get_direct_context(captures, language, lsp, project_dir, rel_file):
    ...
    for ref in captures:
        ...
        try:
            symbol_definition = await lsp.request_definition(
...
async def get_reverse_context(captures, lsp, project_dir, rel_file):
    ...
    for ref in captures:
        ...
        try:
            symbol_references = await lsp.request_references(
                rel_file, line=ref[0].start_point[0], column=ref[0].start_point[1]
            )
        except:
            symbol_references = []
