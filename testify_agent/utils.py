...
from testify_agent.lsp_logic.utils.utils import is_forbidden_directory
from testify_agent.version import __version__
...
def parse_args_full_repo():
    ...
    """
    parser = argparse.ArgumentParser(description=f"Testify Agent v{__version__}")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If set, testify-agent will look for the oldest unchanged test file to analyze.",
    )
    ...
    parser.add_argument(
        "--strict-coverage",
        action="store_true",
        help="If set, testify-agent will return a non-zero exit code if the desired code coverage is not achieved. Default: False.",
    )
    parser.add_argument(
        "--run-tests-multiple-times",
        type=int,
        default=1,
        help="Number of times to run the tests generated by Testify Agent. Default: %(default)s.",
    )
    parser.add_argument(
